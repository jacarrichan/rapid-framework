...
public class TableTest extends TestCase {
	...
	public void testTable() throws Exception {
		...
		System.out.println("枚举列大小...." + t.getEnumColumns().size());
	}
	...
	private void printForTableConfig(Column o) throws IllegalAccessException,
			InvocationTargetException, NoSuchMethodException {
		Map map = BeanHelper.describe(o);
		System.out.println("|| *属性* || *描述* || *示例值* ||");
		for (Object key : map.keySet()) {
			if (map.get(key) instanceof Boolean) {
				System.out.println(String.format("<%s>${c.%s?string}</%s>",
						key, key, map.get(key)));
			} else {
				System.out.println(String.format("<%s>${c.%s!}</%s>", key, key,
						map.get(key)));
			}
		}
	}
	...
	public void test_remove_table_prefix() {
		GeneratorProperties.setProperty(
				GeneratorConstants.TABLE_REMOVE_PREFIXES, "t_,v_");
	...
	public void test_alipay_dalgen_rule() {
		GeneratorProperties.setProperty(
				GeneratorConstants.TABLE_NAME_SINGULARIZE, "true");
		Table table = new Table();
		table.setSqlName("bashes");
		// assertEquals("dalgen是这种规则","Bashe",table.getClassName()); //TODO
		// dalgen是这种规则
	}
	...
	public void test_TABLE_NAME_SINGULARIZE() {
		...
		GeneratorProperties.setProperty(
				GeneratorConstants.TABLE_NAME_SINGULARIZE, "false");
		table.setSqlName("bashes");
